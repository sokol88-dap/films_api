...
class PopulateDB(Resource):
    ...
    def get_films_urls(self):
        ...
        movie_links = [
            movie.a.attrs['href'] for movie in movie_containers][:5]
        return movie_links
...
class PopulateDBThreaded(Resource):
    ...
    def post(self):
        ...
        for film_url in films_urls:
            threads.append(threading.Thread(target=self.parse_films,
        ...
        dt = datetime.datetime.now() - t0
        print(f'Done in {dt.total_seconds():.2f} sec.')
        return {'message': f'Database was populated with {created_films} films.'}, 201
    ...
    def get_films_urls(self):
        ...
        movie_containers = soup.find_all('td', class_='posterColumn')
        movie_links = [
            movie.a.attrs['href'] for movie in movie_containers][6:11]
        return movie_links
...
class PopulateDBThreadedPoolExecutor(Resource):
    url = 'https://www.imdb.com/'
